...
public class Orbit : MonoBehaviour
{
    ...
    public Vector3 dir, forward, side, cross, formation3;
    Rigidbody rb;
    public bool Orbiting;
    GameObject orbitPoint;
    private int formationindex = 0;
    public Vector3[] formationarray;
    int crystalClockwise = 1;
    public Vector3 diagonal;
    public bool even;
    public bool locked;

    public Vector3 lockStart;
    public Vector3 lockEnd;
    float lockSpeed = 20f;
    ...
    void Start()
    {
        ...
        locked = false;
        //diagonal = new Vector3((float)crystalClockwise, 0, Random.Range(.5f, .75f));
        //reversediagonal = new Vector3((float)-crystalClockwise, 0, Random.Range(.5f, .75f));
        ...
        formationarray = new[] { new Vector3(0, crystalClockwise, 0), new Vector3(crystalClockwise, 0, 0), new Vector3(crystalClockwise, 0, Random.Range(.5f, .75f)) };
        side = formationarray[0];
    }
    ...
    void Update()
    {
        if (even == true )
        {
            ...
            formationarray[2] = new Vector3(formationarray[2].x, formationarray[2].y, formationarray[2].z);
        }
        else
        {
            formationarray[2] = new Vector3(-formationarray[2].x, formationarray[2].y, formationarray[2].z);
        ...
        {
            formationindex++;
            if (formationindex >= formationarray.Length)
            {
                formationindex = 0;
            }
        }
        ...
        {
          // Debug.Log("This many crystals in the list" + manager.playerStones.Count + " " + manager.playerStones.Count % 2);
        }

        //Debug.Log(formationindex);

        side = formationarray[formationindex];
    }
    ...
    public void SetUpOrbit()
    {
        ...
        manager.playerStones.Add(gameObject);//Adds each stone to the list
        //print(manager.playerStones.Count);
        manager.idSetter();
